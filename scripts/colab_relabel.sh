#!/bin/bash
# Usage: scripts/colab_relabel.sh MODEL_NAME LOGFILE
# Probes MODEL_NAME (ckpt volume) against the wiki fact set on a Colab A100 with
# ufakzeka.data.boundary_relabel, uploads facts/wiki_facts_probed.jsonl to the data volume, stops the session.
set -u
NAME=$1; LOG=$2
S=relabel
...
sh('modal volume get ufakzeka-data facts/wiki_facts.jsonl /content/wiki_facts.jsonl --force >/dev/null')
os.chdir('/content/w')
sh('python -u -m ufakzeka.data.boundary_relabel --model /ckpt/m --facts /content/wiki_facts.jsonl --out /content/wiki_facts_probed.jsonl --k 3 --n 16000 --batch 64')
sh('modal volume put ufakzeka-data /content/wiki_facts_probed.jsonl facts/wiki_facts_probed.jsonl --force')
...
while true; do
  ...
  [ -n "$NEW" ] && echo "$NEW" >> "$LOG"
  last=$(echo "$OUT" | wc -l)
  echo "$OUT" | grep -q "RELABEL_DONE" && break
